...
public class StudyModifyCommand implements StudyCommand {
	...
	public int execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(dao.updatePost(p_title, content, fileUrl, updateAt, postId) == 1) {
			writer.println("<script>alert('게시물이 수정되었습니다.'); location.href='board_list_Study.doStudy?title="+boardId+"';</script>");
			writer.close();
			return 1;
		}else {
